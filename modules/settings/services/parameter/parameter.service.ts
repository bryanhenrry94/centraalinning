...
export class ParameterService {
  ...
  // de la isla 3) default fijo (DEFAULT_PARAMETER, arriba). Así el
  ...
  static getParameterForTenant = async (tenantId: string): Promise<ParameterInput> => {
    ...
    const settings = await SettingsService.getResolvedSettings({
      tenantId,
      jurisdictionId: tenant?.jurisdictionId ?? null,
    });
    ...
    return {
      ...
      digital_file_costs: num("digital_file_costs", DEFAULT_PARAMETER.digital_file_costs),
      extra_administrative_costs: num(
        "extra_administrative_costs",
        DEFAULT_PARAMETER.extra_administrative_costs,
      ),
      report_financial_pricing: num(
        "report_financial_pricing",
        DEFAULT_PARAMETER.report_financial_pricing,
      ),
      blok_check_pricing: num("blok_check_pricing", DEFAULT_PARAMETER.blok_check_pricing),
      blockade_registration_pricing: num(
        "blockade_registration_pricing",
        DEFAULT_PARAMETER.blockade_registration_pricing,
      ),
      far_registration_fee: num("far_registration_fee", DEFAULT_PARAMETER.far_registration_fee),
      bank_account: settings.bank_account || DEFAULT_PARAMETER.bank_account,
      bank_name: settings.bank_name || DEFAULT_PARAMETER.bank_name,
    };
  };
}
