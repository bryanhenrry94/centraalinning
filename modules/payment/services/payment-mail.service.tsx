...
export const sendFinancialReportMail = async (financial_report_id: string) => {
  ...
  // Person.email es opcional (puede quedar vacío, p.ej. deudores
  // importados masivamente y matcheados solo por identification — ver
  // DebtorService.findOrCreate). Debtor.email es el campo obligatorio y
  // confiable, así que es la fuente primaria; Person.email solo sirve de
  // fallback para registros antiguos.
  const to = debtor.email || financial_report.person?.email;
  ...
  // No atrapar el error acá: dejar que se propague para que
  // processFinancialReportPayment lo registre y lo sepa distinguir de un
  // envío exitoso — antes se devolvía un Response.json aquí adentro, lo que
  // ocultaba el fallo real (el llamador nunca veía la excepción).
  if (error) {
