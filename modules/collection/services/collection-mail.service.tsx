...
// [id]/letters/[type]/route.ts, voor de advocaat-Documenten-sectie).
export const buildAanmaningPdfProps = async (
  caseId: string,
): Promise<AanmaningPDFProps> => {
...
export const generateAanmaningPdfBuffer = async (caseId: string) => {
  const params = await buildAanmaningPdfProps(caseId);
  return generatePdfBuffer(<AanmaningPDF {...params} />);
};
...
export const sendAanmaningEmail = async (
  ...
  try {
    ...
    const pdfBuffer = await generateAanmaningPdfBuffer(caseId);
...
export const buildSommatiePdfProps = async (
  caseId: string,
): Promise<SommatiePDFProps> => {
  const claim = await prisma.debtClaim.findUnique({
    where: { id: caseId },
    include: {
      tenant: true,
...
export const generateSommatiePdfBuffer = async (caseId: string) => {
  const params = await buildSommatiePdfProps(caseId);
  return generatePdfBuffer(<SommatiePDF {...params} />);
};
...
export const sendSommatieEmail = async (to: string, caseId: string) => {
  try {
    ...
    const pdfBuffer = await generateSommatiePdfBuffer(caseId);
    const pdfBase64 = pdfBuffer.toString("base64");
...
export const buildIngebrekestellingPdfProps = async (
  caseId: string,
): Promise<IngebrekestellingProps> => {
  const claim = await prisma.debtClaim.findUnique({
    where: { id: caseId },
    include: {
      tenant: true,
...
export const generateIngebrekestellingPdfBuffer = async (caseId: string) => {
  const params = await buildIngebrekestellingPdfProps(caseId);
  return generatePdfBuffer(<IngebrekestellingPDF {...params} />);
};
...
export const sendIngebrekestellingMail = async (to: string, caseId: string) => {
  try {
    ...
    const pdfBuffer = await generateIngebrekestellingPdfBuffer(caseId);
    const pdfBase64 = pdfBuffer.toString("base64");
